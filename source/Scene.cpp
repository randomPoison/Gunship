...
//ComponentInfo Gunship::Scene::AddGameObject( const char* name )
//{
//	gameObjects.emplace_back( _sceneManager->getRootSceneNode()->createChildSceneNode(), name );
//	return ComponentInfo{ gameObjects.back().id, gameObjects.size() - 1 };
//}
//
//void Gunship::Scene::AddCameraComponent( ComponentInfo& gameObject )
//{
//	GameObjectComponent* owner = FindGameObject( gameObject );
//	Ogre::Camera* camera = _sceneManager->createCamera( std::to_string( gameObject.id ) );
//	owner->node->attachObject( camera );
//	Ogre::Viewport* viewport = _renderWindow->addViewport( camera );
...
//}
//
//void Gunship::Scene::AddMesh( ComponentInfo& gameObject, const char* mesh )
//{
//	GameObjectComponent* owner = FindGameObject( gameObject );
//	Ogre::Entity* cubeEntity = _sceneManager->createEntity( MeshID( mesh, *owner ).c_str(), mesh );
//	cubeEntity->setMaterialName( "Test/ColourTest" );
//	owner->node->attachObject( cubeEntity );
//}
//
//GameObjectComponent* Gunship::Scene::FindGameObject( ComponentInfo& gameObjectInfo )
//{
